function OrderEditController($location, $routeParams, orderStorage) {
  var order = this;
  ...
  order.save = function (){
    storage.setItem(order.id, order.name).
      then(function(data){
        console.log('Order ' + data + ' saved in database');
...
OrderEditController.$inject = ['$location', '$routeParams', 'orderStorage'];

function OrderNewController($location, orderStorage) {
  var order = this;
  var storage = orderStorage.load();
...
OrderNewController.$inject = ['$location', 'orderStorage'];
